...
public class LabOrdersPageController {
    ...
    public void get(PageModel model, UiUtils ui,
                      ...
                      @RequestParam(value = "testConcept", required = false) Concept testConcept,
        ...
        Set<Concept> testConcepts = new TreeSet<>(Comparator.comparing(labOrderConfig::formatConcept));
        for (Order order : orderService.getAllOrdersByPatient(patient)) {
            ...
            if (order instanceof TestOrder && BooleanUtils.isNotTrue(order.getVoided())) {
                testConcepts.add(order.getConcept());
                if (testConcept == null || testConcept.equals(order.getConcept())) {
        ...
        model.addAttribute("labOrderType", labOrderConfig.getLabTestOrderType());
        model.addAttribute("testConcepts", testConcepts);
        model.addAttribute("testConcept", testConcept);
    }
}
