...
public class TermsAndConditionsFilter implements Filter {
	...
	public static final String SESSION_ATTRIBUTE_TERMS_ACCEPTED = "pihapps.termsAndConditionsAccepted";

	public static final String TERMS_URI = "/" + WebConstants.CONTEXT_PATH + "/pihapps/account/termsAndConditions.page";
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		try {
			enabled = "true".equalsIgnoreCase(ConfigUtil.getProperty(TERMS_AND_CONDITIONS_ENABLED_PROPERTY, "false"));
		}
		catch (Exception e) {
		...
		if (enabled && request instanceof HttpServletRequest && response instanceof HttpServletResponse) {
			...
			try {
				...
				if (!"true".equals(httpRequest.getSession().getAttribute(SESSION_ATTRIBUTE_TERMS_ACCEPTED))) {
					...
					if (currentUser != null) {
						String propertyVal = currentUser.getUserProperty(USER_PROPERTY_TERMS_AND_CONDITIONS_ACCEPTED_DATE);
